...
function onJoin()
	...
	if accountCheck == 1 then
		if avatar ~= "none" then
			triggerClientEvent(client, "openLoginPanel", client, accountCheck, getPlayerSerial(client), base64Decode(avatar))
		else
			triggerClientEvent(client, "openLoginPanel", client, accountCheck, getPlayerSerial(client), false)
		end
